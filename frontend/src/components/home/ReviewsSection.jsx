...
const ReviewsSection = ({ reviews, loading }) => {
  // Ensure we have at least 10 reviews for a smooth, gap-free infinite scroll effect
  const reviewsCount = reviews?.length || 0;
  const minReviewsNeeded = 10;
  ...
  const {
    ...
    speed: 0.5, // slow and smooth scroll, perfect for reading testimonials
              ...
              {doubledReviews.map((review, idx) => {
                ...
                return (
                  <div key={`${review._id}-${idx}`} className="px-4 shrink-0">
                    <div
                      className="relative group transition-all duration-300 rounded-2xl border border-slate-100 bg-white shadow-panel overflow-hidden w-[320px] sm:w-[580px] h-[340px] sm:h-[280px]"
                    >
                      ...
                      <div className={`grid h-full ${hasImage ? 'grid-cols-[1.35fr,0.65fr]' : 'grid-cols-1'}`}>
